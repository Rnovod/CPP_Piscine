...
void	GameZone::game(void) {
	...
	while (42) {
		...
		if (_delay % 20 == 0 && _numEnemy < _maxEnemy) {
			_numEnemy++;
			createEnemy();
		}
		// wclear(_win);
		_stars.drawStars(_height, _width, _win, stars_delay);
		set_panel(_Entities, _level, _score);//TODO changed
		drawEntities();
		if (_god) {
		...
		wrefresh(_win);
		stars_delay++;
		stars_delay %= (_level == 1) ? 3 : 4;
		stars_delay %= (_level == 2) ? 2 : 3;
		stars_delay %= (_level == 3) ? 1 : 2;
		if (stars_delay > 1000)
			stars_delay = 0;
		usleep(25000);
...
void	GameZone::drawMenu(int menukey) {
	...
	if (menukey == 3) {
		mvwprintw(_win, _height / 2 + 1, _width / 2 - 2, ">QUIT");
	} else {
		mvwprintw(_win, _height / 2 + 1, _width / 2 - 2, " QUIT");
	}
